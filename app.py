...
def apply_style() -> None:
    st.markdown(
        ...
        .gold-rain .gold-bar {
            position:absolute; top:-9vh; left:var(--left); width:calc(38px * var(--scale)); height:calc(22px * var(--scale));
            opacity:0; transform-style:preserve-3d; will-change:transform,opacity;
            clip-path:polygon(15% 0,85% 0,100% 31%,92% 100%,8% 100%,0 31%);
            border-radius:4px;
            background:linear-gradient(155deg,#fff3a3 0%,#ffd34d 20%,#e3a315 52%,#b97805 78%,#f1bd2a 100%);
            box-shadow:inset 0 -5px 8px rgba(108,67,0,.24),inset 0 3px 5px rgba(255,250,193,.62),0 5px 12px rgba(82,53,0,.18);
            animation:goldBarFall var(--duration) cubic-bezier(.35,.05,.65,.95) infinite; animation-delay:var(--delay);
        }
        .gold-rain .gold-bar::before {
            content:"Au"; position:absolute; inset:3px 7px auto; height:42%; display:flex; align-items:center; justify-content:center;
            color:#8b5a00 !important; font-family:Georgia,serif; font-size:calc(8px * var(--scale)); font-weight:800; letter-spacing:.03em;
            line-height:1; border-radius:3px; border:1px solid rgba(137,87,0,.24); background:rgba(255,236,126,.24);
            text-shadow:0 1px 0 rgba(255,244,170,.70);
        }
        .gold-rain .gold-bar::after {
            content:""; position:absolute; left:15%; right:15%; top:0; height:27%; border-radius:3px 3px 1px 1px;
            background:linear-gradient(90deg,rgba(255,255,221,.72),rgba(255,220,77,.30),rgba(255,248,177,.72));
        }
        ...
        @keyframes goldBarFall {
            0% { opacity:0; transform:translate3d(0,-12vh,0) rotateX(18deg) rotateY(0deg) rotateZ(-12deg); }
            9% { opacity:.46; }
            46% { transform:translate3d(var(--drift),48vh,0) rotateX(210deg) rotateY(175deg) rotateZ(95deg); }
            72% { transform:translate3d(0,77vh,0) rotateX(325deg) rotateY(285deg) rotateZ(168deg); }
            88% { opacity:.40; }
            100% { opacity:0; transform:translate3d(var(--drift),112vh,0) rotateX(440deg) rotateY(390deg) rotateZ(235deg); }
        }
        ...
        <div class="gold-rain" aria-hidden="true">
          <span class="gold-bar" style="--left:20%;--delay:-3s;--duration:18s;--scale:.80;--drift:34px"></span>
          <span class="gold-bar" style="--left:31%;--delay:-13s;--duration:22s;--scale:.62;--drift:-42px"></span>
          <span class="gold-bar" style="--left:42%;--delay:-7s;--duration:20s;--scale:.92;--drift:48px"></span>
          <span class="gold-bar" style="--left:54%;--delay:-18s;--duration:25s;--scale:.66;--drift:-28px"></span>
          <span class="gold-bar" style="--left:66%;--delay:-10s;--duration:21s;--scale:.76;--drift:40px"></span>
          <span class="gold-bar" style="--left:77%;--delay:-1s;--duration:23s;--scale:.58;--drift:-36px"></span>
          <span class="gold-bar" style="--left:87%;--delay:-15s;--duration:24s;--scale:.84;--drift:44px"></span>
          <span class="gold-bar" style="--left:96%;--delay:-6s;--duration:20s;--scale:.64;--drift:-24px"></span>
        </div>
        """,
