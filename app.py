...
def apply_style() -> None:
    st.markdown(
        ...
        /* ---------- Subtle falling gold coins ---------- */
        .gold-rain {
            position:fixed; inset:0; overflow:hidden; pointer-events:none; z-index:999;
            user-select:none; contain:strict;
        }
        .gold-rain .gold-coin {
            position:absolute; top:-8vh; left:var(--left); width:calc(21px * var(--scale)); height:calc(21px * var(--scale));
            border-radius:50%; opacity:0; transform-style:preserve-3d; will-change:transform,opacity;
            border:1px solid rgba(157,101,0,.72);
            background:radial-gradient(circle at 34% 27%,#fff7bd 0 8%,#ffd65a 22%,#e5a91b 58%,#a96f05 100%);
            box-shadow:inset 0 0 0 2px rgba(255,241,148,.42),0 4px 9px rgba(82,53,0,.17);
            animation:coinFall var(--duration) linear infinite; animation-delay:var(--delay);
        }
        .gold-rain .gold-coin::before {
            content:"Au"; position:absolute; inset:0; display:flex; align-items:center; justify-content:center;
            color:#8b5a00 !important; font-family:Georgia,serif; font-size:calc(8px * var(--scale)); font-weight:800;
            line-height:1; text-shadow:0 1px 0 rgba(255,244,170,.65);
        }
        .gold-rain .gold-coin::after {
            content:""; position:absolute; inset:3px; border-radius:50%; border:1px solid rgba(137,87,0,.28);
        }
        ...
        @keyframes coinFall {
            ...
            8% { opacity:.44; }
            88% { opacity:.38; }
            100% { opacity:0; transform:translate3d(var(--drift),112vh,0) rotateY(760deg) rotateZ(420deg); }
        }
        ...
        @media (max-width: 760px) {
            ...
            .gold-rain .gold-coin:nth-child(n+7) { display:none; }
        }
        ...
        <div class="gold-rain" aria-hidden="true">
          <span class="gold-coin" style="--left:19%;--delay:-2s;--duration:15s;--scale:.82;--drift:30px"></span>
          <span class="gold-coin" style="--left:27%;--delay:-11s;--duration:18s;--scale:.65;--drift:-40px"></span>
          <span class="gold-coin" style="--left:36%;--delay:-5s;--duration:16s;--scale:.90;--drift:45px"></span>
          <span class="gold-coin" style="--left:44%;--delay:-14s;--duration:21s;--scale:.68;--drift:-20px"></span>
          <span class="gold-coin" style="--left:52%;--delay:-8s;--duration:17s;--scale:.78;--drift:36px"></span>
          <span class="gold-coin" style="--left:61%;--delay:-1s;--duration:19s;--scale:.62;--drift:-38px"></span>
          <span class="gold-coin" style="--left:68%;--delay:-12s;--duration:16s;--scale:.88;--drift:30px"></span>
          <span class="gold-coin" style="--left:76%;--delay:-6s;--duration:20s;--scale:.72;--drift:-25px"></span>
          <span class="gold-coin" style="--left:83%;--delay:-16s;--duration:22s;--scale:.58;--drift:42px"></span>
          <span class="gold-coin" style="--left:89%;--delay:-3s;--duration:17s;--scale:.82;--drift:-36px"></span>
          <span class="gold-coin" style="--left:94%;--delay:-10s;--duration:20s;--scale:.70;--drift:24px"></span>
          <span class="gold-coin" style="--left:98%;--delay:-7s;--duration:18s;--scale:.62;--drift:-18px"></span>
        </div>
        """,
