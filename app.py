...
def apply_style() -> None:
    st.markdown(
        ...
        html, body, .stApp, [data-testid="stAppViewContainer"] { background:#f4f7fb !important; color:#111827 !important; }
        ...
        /* ---------- Sidebar ---------- */
        [data-testid="stSidebar"] { background:#0b1a33 !important; border-right:1px solid rgba(255,255,255,.06); }
        ...
        [data-testid="stSidebar"] .sb-dot { width:.6rem; height:.6rem; border-radius:50%; flex:0 0 auto; }
        ...
        button[data-baseweb="tab"] { border-radius:10px; padding:.5rem 1.05rem; }
        ...
        .hero { position:relative; overflow:hidden; padding:2.1rem 2.4rem; border:1px solid rgba(255,255,255,.14); border-radius:22px;
            background:linear-gradient(120deg,#071426 0%,#0e2a5e 52%,#1d4ed8 100%); box-shadow:0 16px 38px rgba(15,23,42,.20); color:#ffffff !important; margin-bottom:1.3rem; }
        .hero::before { content:""; position:absolute; width:300px; height:300px; right:-70px; top:-140px; border-radius:50%;
            background:radial-gradient(circle,rgba(251,191,36,.32),rgba(251,191,36,0) 68%); }
        .hero::after { content:""; position:absolute; width:210px; height:210px; right:16%; bottom:-160px; border-radius:50%; background:rgba(96,165,250,.20); filter:blur(4px); }
        ...
        /* ---------- KPI strip ---------- */
        div[data-testid="stMetric"] { background:#ffffff !important; border:1px solid #dfe6ee; padding:.95rem 1rem; border-radius:14px; box-shadow:0 3px 10px rgba(15,23,42,.05); }
        ...
        .data-table th { background:#0f172a; color:white; padding:.62rem .68rem; text-align:left; white-space:nowrap; }
...
def sidebar(data: dict, contracts: dict) -> str:
    with st.sidebar:
        ...
        with st.expander("How this app works"):
            st.markdown(
                "- **Existing Evaluation Date** replays saved walk-forward predictions.\n"
                "- **Manual Input** calls the four saved, fitted deployment pipelines.\n"
                "- Nothing is fitted, tuned or resplit inside this app.",
            )
...
def overview_kpis(data: dict, contracts: dict) -> None:
    ...
    canonical = data["canonical"]
    date_range = f'{canonical["Origin_Date"].min():%Y-%m-%d} → {canonical["Origin_Date"].max():%Y-%m-%d}'

    columns = st.columns(4)
    columns[0].metric("Best overall model", contracts["best_model"])
    columns[1].metric("Overall Price RMSE", f'{winner["Overall_Price_RMSE"]:,.2f}')
    columns[2].metric("RMSE skill vs persistence", f'{winner["Overall_RMSE_Skill_vs_Persistence"] * 100:.2f}%')
    columns[3].metric("Evaluation window", date_range)
