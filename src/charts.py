...
def actual_vs_predicted_figure(
    ...
    data = model_predictions.loc[model_predictions["Horizon"].eq(horizon)].sort_values("Target_Date")
    ...
    actual_history = (
        model_predictions[["Target_Date", "Actual_Price"]]
    ...
    focus_end = None
    figure.add_trace(go.Scatter(
        x=data["Target_Date"], y=data["Predicted_Price"],
        name=f"{model} historical prediction",
        line={"color": COLORS.get(model), "width": 1.8},
    ))
    figure.add_trace(go.Scatter(
        x=data["Target_Date"], y=data["Persistence_Price"],
        name="Persistence",
        line={"color": "#6B7280", "width": 1.5, "dash": "dot"},
    ))
    # Draw Actual last so the final dataset records remain visible above the
    # historical prediction and persistence lines.
    figure.add_trace(go.Scatter(
        x=actual_history["Target_Date"], y=actual_history["Actual_Price"],
        name="Actual",
        line={"color": "#111827", "width": 2.6},
    ))
    ...
    if future_results is not None and not future_results.empty and forecast_origin_date is not None:
        ...
        # Continue from the final real dataset point to H1 in one uninterrupted
        # trace. The first point is an existing Actual record, not an H0 forecast.
        ...
        figure.add_trace(go.Scatter(
            ...
            customdata=[["Last dataset record"]] + [[horizon_name] for horizon_name in future["Horizon"]],
            hovertemplate="%{customdata[0]}<br>Price: %{y:,.2f}<extra>%{fullData.name}</extra>",
            connectgaps=True,
