...
def actual_vs_predicted_figure(
    ...
    if future_results is not None and not future_results.empty and forecast_origin_date is not None:
        ...
        future_color = COLORS.get(model)

        # Visually continue from the final real dataset point to H1 without
        # creating or labelling an extra H0 forecast record.
        final_actual = data.iloc[-1]
        figure.add_trace(go.Scatter(
            x=[final_actual["Target_Date"], future_dates[0]],
            y=[final_actual["Actual_Price"], float(future.iloc[0]["Predicted Price"])],
            mode="lines",
            showlegend=False,
            hoverinfo="skip",
            line={"color": future_color, "width": 3, "dash": "dash"},
        ))
        figure.add_trace(go.Scatter(
            x=future_dates.tolist(),
            y=future["Predicted Price"].tolist(),
            mode="lines+markers",
            name=f"{model} future H1–H7",
            customdata=[[horizon_name] for horizon_name in future["Horizon"]],
            hovertemplate="%{customdata[0]}<br>Price: %{y:,.2f}<extra>%{fullData.name}</extra>",
            line={"color": future_color, "width": 3, "dash": "dash"},
            marker={
                "size": 6,
